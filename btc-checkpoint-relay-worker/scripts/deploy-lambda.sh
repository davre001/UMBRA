#!/usr/bin/env bash
# Deploys btc-checkpoint-relay-worker to AWS Lambda (zip package, no
# container/ECR needed) and wires an EventBridge schedule to invoke it every
# POLL_MINUTES minutes. Idempotent: safe to re-run after a code change
# (updates function code) or a config change (updates config).
#
# Own function and schedule, deliberately NOT btc-deposit-worker's or
# matcher-worker's Lambda: signet averages ~10min blocks, so POLL_MINUTES
# here defaults to 10 — no point checking more often than the real chain
# actually changes.
#
...
#   FUNCTION_NAME     default umbra-btc-checkpoint-relay-worker
#   POLL_MINUTES      default 10
...
FUNCTION_NAME="${FUNCTION_NAME:-umbra-btc-checkpoint-relay-worker}"
...
# synchronous PUT) — same approach the other workers' deploy scripts use.
