#!/usr/bin/env bash
# Deploys btc-deposit-worker to AWS Lambda (zip package, no container/ECR
# needed) and wires an EventBridge schedule to invoke it every POLL_MINUTES
# minutes. Idempotent: safe to re-run after a code change (updates function
# code) or a config change (updates config).
#
# Own function and schedule, deliberately NOT matcher-worker's Lambda: that
# one polls every 5 minutes because match_orders benefits from batching;
# BTC deposits have no such reason to wait, so POLL_MINUTES here defaults
# to 1 — EventBridge's own minimum granularity for a rate() expression.
#
...
#   BTC_DEPOSIT_INTERNAL_SECRET  must match the backend's own BTC_DEPOSIT_INTERNAL_SECRET
# Optional:
#   AWS_REGION       default us-east-1
#   FUNCTION_NAME     default umbra-btc-deposit-worker
...
: "${BTC_DEPOSIT_INTERNAL_SECRET:?Set BTC_DEPOSIT_INTERNAL_SECRET (must match the backend's own value)}"
AWS_REGION="${AWS_REGION:-us-east-1}"
FUNCTION_NAME="${FUNCTION_NAME:-umbra-btc-deposit-worker}"
...
# The zip is tens of MB (bundles bb.js's WASM binary) — the direct
# `--zip-file fileb://...` upload path (whole file in one HTTP request
# body) is a known reliability issue for large inline Lambda uploads.
# Uploading to S3 first and deploying via --code S3Bucket/S3Key instead is
# the standard, more resilient fix (chunked S3 upload vs. one big
# synchronous PUT) — same approach matcher-worker's deploy script uses.
...
# every proving attempt would fail with "Read-only file system" without
# this. /tmp is the one writable path in a Lambda execution environment.
# No corresponding code change needed in prove.ts — bb.js reads CRS_PATH
# from process.env internally (confirmed via source inspection).
ENV_VARS="Variables={BACKEND_URL=${BACKEND_URL},BTC_DEPOSIT_INTERNAL_SECRET=${BTC_DEPOSIT_INTERNAL_SECRET},CRS_PATH=/tmp/.bb-crs}"
...
echo "Polling every ${POLL_MINUTES} minute(s) via rule $RULE_NAME."
