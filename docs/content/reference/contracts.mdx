...
\#6. **Current status**: `executeSetTrustedVerifier`/`executeSetExternalDepositToken`
have now executed. The checkpoint genesis bootstrap is queued (targeting
signet height 320241) with its 48h window closing 2026-09-03 — until that
executes, BTC deposits/withdrawals aren't live on this vault — see the root
[README's Status section](https://github.com/davre001/UMBRA#status) for
the up-to-date picture. Before that, `ShieldedVault` was separately redeployed (2026-08-11) to
