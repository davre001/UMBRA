...
\#6. **Current status**: the queued `executeSetTrustedVerifier`/
`executeSetExternalDepositToken` calls and the checkpoint genesis bootstrap
haven't run yet, so BTC deposits/withdrawals aren't live on this vault —
see the root [README's Status section](https://github.com/davre001/UMBRA#status)
for the up-to-date picture. Before that, `ShieldedVault` was separately redeployed (2026-08-11) to
