...
calls the per-circuit verifiers. `OwnerKeyRegistry` and `StealthAnnouncer`
are standalone helpers read and written entirely off-chain by clients, and
`ComplianceRegistry` is only consulted by the vault on `withdraw`.
