...
> **Checkpoint trust:** The circuit does not validate Bitcoin full history back to genesis — it trusts one recent, admin-registered header as its root of trust, the same model BTC Relay and SPV clients use, and proves only that a 6-header chain (K = 6, roughly an hour of signet blocks) correctly extends forward from it to the deposit confirming block. Whoever controls what gets registered as the checkpoint is trusted not to register one from a forged or minority-fork chain.
...
> **Honest current limitation:** A deposit can only be proven once the on-chain checkpoint is refreshed to align with its exact confirming block height. That refresh is currently a manual, admin-run script, not an automated poll loop — so a real deposit can sit fully confirmed but unminted until an admin runs it.
